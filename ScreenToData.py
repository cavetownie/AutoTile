...
class winInteract:
    def __init__(self, name=None, cutoff=1):
        if cutoff < 1:
            raise ValueError("The cutoff cannot be less than one!")

        """
        Find the process handler for the winHwnd using the name
        specified by user in main. 
        """
        self.winHwnd = win32gui.FindWindowEx(None,None,None,name)
        if not self.winHwnd:
        ...
        self.height = int((self.pos[3] - self.pos[1])/cutoff) # We don't need the entire application 
